...
import { generateTextPrompt, generateMultipleChoicePrompt, CHOSEN_DEEPSEEK_MODEL } from "~/server/util/ai"; // prettier-ignore
    ...
    try {
        ...
        if (unresolvedTextQuestions.length) {
            const { text: textAnswers } = await ai.generate({
                model: deepSeek.model(CHOSEN_DEEPSEEK_MODEL, {
                    temperature: 0,
                }),
